...
jQuery(document).ready(function() {
	...
	metabox_select_icon ();
	oswald_custom_field_element();
});


function metabox_tab(){
	var title,id,showing_field;
	jQuery( '#advanced-sortables' ).wrapInner( "<div class='wpd_tab_meta_container'></div>" );
	jQuery( '#advanced-sortables' ).append('<div class="wpd_tab_meta_titles"></div>')
	jQuery('#advanced-sortables').find('.postbox ').each(function(){
		title = jQuery(this).children('h2').children('span').clone();
		id = jQuery(this).attr('id');
		jQuery('#advanced-sortables').find('.wpd_tab_meta_titles').append('<h2 class="'+id+'">'+(title.text())+'</h2>');
	})

	jQuery('.wpd_tab_meta_container').children().hide();
	jQuery('.wpd_tab_meta_container').find('.postbox:first-child').show();
	jQuery('.wpd_tab_meta_titles h2:first-child').addClass('active');
	jQuery('#advanced-sortables').find('.wpd_tab_meta_titles h2').on("click",function(){
...
function metabox_dependancy (){
	...
	for (var i = dependencyArrayLength - 1; i >= 0; i--) {
		...
		if (typeof(eachElement[parentDepend]) == 'undefined') {
			var valuee = [operationDepend, valueDepend] 
			var metabox_parent = {};
			metabox_parent['properties'] = [valuee];
			var ret = metabox_parent;
...
function oswald_custom_field_element(){
