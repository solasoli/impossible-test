<?php 
// header functions
if (!function_exists('oswald_header_builder__container')) {
    function oswald_header_builder__container($options = null) {
    	extract($options);
		                        ...
		                        foreach ($wpd_header_builder_array[$side]['content'] as $key => $value) {
		                            if ($key != 'placebo' && $key != 'undefined') {
		                                    ...
		                                    switch ($key) {
		                                        ...
		                                        case 'menu':
		                                            if (!empty($menu_slug)) {
		                                                oswald_get_template_part( 'template-parts/header/header_part/menu', '', array('menu_slug' => $menu_slug, 'menu_ative_top_line' => $menu_ative_top_line, 'menu_mobile_nav' => true));
		                                                $is_header_menu = true;
		                                            }
		                                            break;
		                                        case 'button':
		                                            ...
		                                            break;
		                                        case 'search':
		                                        	$show_search = oswald_option('show_search');
		                                        	if (!class_exists('wpdaddy_core') && !$show_search) {
		                                        		break;
		                                        	}
		                                            echo '<div class="wpd_header_builder_component wpd_header_builder_search_component">
		                                            		...
		                                            		<div class="header_search__container">
										                    	...
										                    	<div class="header_search__inner_close"><i class="header_search__search_close_icon"></i></div></div></div></div></div>';
		                                            break;
		                                        case 'login':
		                                            $is_login = true;
		                                        ...
		                                        case 'wpml':
                                                    if ( in_array( 'sitepress-multilingual-cms/sitepress.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
                                                    	echo '<div  class="wpd_header_builder_component wpd_header_builder_wpml_component">';
		                                        ...
		                                        case 'cart':
		                                            if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
		                                            	$show_cart = oswald_option('show_cart');
			                                        	if (!class_exists('wpdaddy_core') && !$show_cart) {
			                                        		break;
			                                        	}
		                                        ...
		                                        case 'burger_sidebar':
		                                            ...
		                                            break;
		                                        case 'text1':
		                                        	$show_text1 = oswald_option('show_custom_text');
		                                        	if (!class_exists('wpdaddy_core') && !$show_text1) {
		                                        		break;
		                                        	}
